...
class KotlinFileVisitor {
    ...
    private fun visitStructured(
        ...
        if (structured.typeParams.isNotEmpty()) {
            description.append('<')
            description.append(structured.typeParams.joinToString(", ") {
                if (it.type != null) {
        ...
        val parents = structured.parents.joinToString(", ") {
            when (it) {
        ...
        val primary = structured.primaryConstructor?.asConstructor()
        primary?.let { visitDeclaration(it, pkg, level + 1) }
    ...
    private fun visitFunction(func: Node.Decl.Func) {
        ...
        val typeParams = func.typeParams.joinToString(", ") {
            if (it.type == null) {
                it.name
            } else {
                "${it.name}: ${it.type.description()}"
        ...
        if (typeParams.isNotBlank()) {
            description.append('<')
            description.append(typeParams)
            description.append("> ")
        }
