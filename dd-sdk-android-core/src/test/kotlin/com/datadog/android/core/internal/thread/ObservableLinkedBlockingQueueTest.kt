...
class ObservableLinkedBlockingQueueTest {
    ...
    fun `M return only once non-null map W try to dump multiple times in dump interval`(
        ...
        val testedObservableLinkedBlockingQueue =
            ObservableLinkedBlockingQueue<Any>(fakeItemCount + 1)
        ...
        // When
        val firstMap = testedObservableLinkedBlockingQueue.dumpQueue(fakeTimestamps.next())
        val secondMap = testedObservableLinkedBlockingQueue.dumpQueue(fakeTimestamps.next())
        val thirdMap = testedObservableLinkedBlockingQueue.dumpQueue(fakeTimestamps.next())
    ...
    fun `M build correct map W try to dump named runnable`(
        ...
        val fakeTimestamp = forge.aLong(min = 10000L)
        val fakeRunnableCount = forge.anInt(min = 5, max = 100)
        val testedObservableLinkedBlockingQueue =
            ObservableLinkedBlockingQueue<Runnable>(fakeRunnableCount + 1)
        ...
        repeat(fakeRunnableCount) {
            ...
            val fakeNamedRunnable = NamedRunnable(fakeName, mock())
            testedObservableLinkedBlockingQueue.offer(fakeNamedRunnable)
            expectedMap[fakeName] = (expectedMap[fakeName] ?: 0) + 1
        }

        // When
        val map = testedObservableLinkedBlockingQueue.dumpQueue(fakeTimestamp)
