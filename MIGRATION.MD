...
> [!WARNING] 
> Note that the `OpenTelemetry` specification library [requires](https://github.com/open-telemetry/opentelemetry-java?tab=readme-ov-file#requirements) desugaring to be enabled for compatibility support. If this requirement cannot be met in your project see the following section.
...
For manual(custom) tracing use `com.datadog.android.trace.GlobalDatadogTracer.get()` instead of `io.opentracing.util.GlobalTracer.get()` to access to the tracer object.
For example: 
```kotlin
...
The OkHttp instrumentation(`com.datadoghq:dd-sdk-android-okhttp:x.x.x`), should work fine. It doesn't require desugaring support either. However few migration actions may be necessary.
