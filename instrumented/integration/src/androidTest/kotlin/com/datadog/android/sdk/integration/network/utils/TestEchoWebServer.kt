...
internal class TestEchoWebServer {
    ...
    private fun handleMethodEndpoint(
        ...
        return jsonResponse(HttpSpec.StatusCode.OK, response)
    }
    ...
    private fun handleErrorEndpoint(path: String, method: String): MockResponse {
        ...
        if (parts.size < PATH_PARTS_MIN_SIZE) {
            return jsonResponse(
                statusCode = HttpSpec.StatusCode.BAD_REQUEST,
                body = ErrorResponse(
                    statusCode = HttpSpec.StatusCode.BAD_REQUEST,
                    method = method,
        ...
        val errorCode = parts[2].toIntOrNull() ?: HttpSpec.StatusCode.INTERNAL_ERROR
    ...
    private fun handleRedirectEndpoint(path: String, method: String): MockResponse {
        ...
        return if (hopCount > 0) {
            ...
            MockResponse()
                .setResponseCode(HttpSpec.StatusCode.FOUND)
                .setHeader(HttpSpec.Header.LOCATION, nextPath)
        } else {
            val response = MethodResponse(
                method = method,
                path = path
            )
            jsonResponse(HttpSpec.StatusCode.OK, response)
        }
    }
    ...
    private fun handleRetryEndpoint(
        ...
        return if (currentCount <= failCount) {
            val retryResponse = RetryResponse(
                statusCode = HttpSpec.StatusCode.TOO_MANY_REQUESTS,
                method = method,
            ...
            MockResponse()
                .setResponseCode(HttpSpec.StatusCode.TOO_MANY_REQUESTS)
                .setHeader(HttpSpec.Header.RETRY_AFTER, "1")
                .setHeader(HttpSpec.Header.CONTENT_TYPE, HttpSpec.ContentType.APPLICATION_JSON)
    ...
    private fun notFoundResponse(): MockResponse {
        return MockResponse().setResponseCode(HttpSpec.StatusCode.NOT_FOUND)
    }

    private fun getHttpStatusMessage(code: Int): String = when (code) {
        HttpSpec.StatusCode.BAD_REQUEST -> STATUS_MESSAGE_BAD_REQUEST
        HttpSpec.StatusCode.UNAUTHORIZED -> STATUS_MESSAGE_UNAUTHORIZED
        HttpSpec.StatusCode.FORBIDDEN -> STATUS_MESSAGE_FORBIDDEN
        HttpSpec.StatusCode.NOT_FOUND -> STATUS_MESSAGE_NOT_FOUND
        HttpSpec.StatusCode.METHOD_NOT_ALLOWED -> STATUS_MESSAGE_METHOD_NOT_ALLOWED
        HttpSpec.StatusCode.REQUEST_TIMEOUT -> STATUS_MESSAGE_REQUEST_TIMEOUT
        HttpSpec.StatusCode.TOO_MANY_REQUESTS -> STATUS_MESSAGE_TOO_MANY_REQUESTS
        HttpSpec.StatusCode.INTERNAL_ERROR -> STATUS_MESSAGE_INTERNAL_ERROR
        HttpSpec.StatusCode.BAD_GATEWAY -> STATUS_MESSAGE_BAD_GATEWAY
        HttpSpec.StatusCode.SERVICE_UNAVAILABLE -> STATUS_MESSAGE_SERVICE_UNAVAILABLE
        HttpSpec.StatusCode.GATEWAY_TIMEOUT -> STATUS_MESSAGE_GATEWAY_TIMEOUT
