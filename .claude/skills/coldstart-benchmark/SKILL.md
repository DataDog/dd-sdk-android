...
which aborted anyway; a run whose whole blocks are not a consecutive prefix from block 1, which
means the file was edited or truncated; an interrupted run passed alongside other CSVs.
`--allow-aborted` inspects any of them diagnostically and suppresses the primary interval.
...
The short version: discarded warm-ups, `am force-stop` (which does **not** evict the page
cache), TTID-only measurement and pre-granted permissions all shrink the number. The default
`COMPILE_FILTER=speed-profile` does not -- a fresh install reaches `verify`, which is pessimistic
rather than flattering; `COMPILE_FILTER=speed` is the one that shrinks it. Animations off
suppresses per-frame SDK work, yet enlarged the total on the app we measured, so do not quote it
as a downward bias without checking. The metric is a **process-cold, page-cache-warm** start to
first frame, and is closer to a lower bound than a worst case. Quote it with that caveat
attached.
