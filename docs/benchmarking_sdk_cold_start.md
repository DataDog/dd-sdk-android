...
Python and will not see the venv. Pass the same flags `capture_trace.sh` passes, or you read
back a weaker verdict than the capture already produced:
...
on a mismatch (`ALLOW_VERSION_MISMATCH=1` acknowledges it if you know why they differ). The same
...
truncated), and an interrupted run pooled with other CSVs, which would let a prefix be chosen to
suit a result. `--allow-aborted` remains available for diagnostic inspection of any of them, but
it suppresses the primary interval.
...
| animations off | frame-count variance during the launch transition | **downward for per-frame SDK work, but upward for the total on the app we measured.** Fewer animated frames means fewer `Choreographer` callbacks for vitals and long-task tracking, and fewer Session Replay snapshots. That is the mechanism; it was not what dominated. On TTFD the delta was +88.6 ms with animations off against +40.2 ms with them on, because animations add slack the SDK's async init overlaps, making animations-off the conservative setting there. See [Disable animations](#disable-animations--but-know-what-that-costs-you). Run `ANIMATIONS=1` and compare rather than assuming either direction |
...
1. **Start Perfetto during the final registered conditioning wait, then immediately `am
   force-stop`, wait five seconds and launch inside the trace window.** Starting it after that wait
   adds another app-running interval; stopping before it omits the final launch boundary. A trace
   with no `bindApplication` slice in the post-force-stop process generation contains no usable
   cold start.
...
| cleanup reports restoration incomplete | one or more restore commands failed, often because the device disconnected during an abort. The benchmark and trace print `device restoration INCOMPLETE` and name every setting, radio, performance/dexopt control or permission to verify by hand; `verify_sdk_active.sh` prints `verifier permission restoration INCOMPLETE` and names each permission it granted and could not revoke, which is left held by the freshly installed app. Each keeps its original exit status, so the verdict above it still stands -- but clear the named state before another run |
