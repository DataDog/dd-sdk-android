...
Note that the interceptor will track requests at the application level. You can also add a `TracingInterceptor` at the Network level to get more details, for example when following redirections.
...
Because the way the OkHttp Request is executed (using a Thread pool), the request span won't be automatically linked with the span that triggered the request. You can still manually provide a parent Span in the `OkHttp Request.Builder` as follows:
