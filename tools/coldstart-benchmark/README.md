...
| `EXPECTED_WARMUP` | *required for trace* | the `warmup` value from the benchmark CSV header. It supplies the trace's `WARMUP` when that variable is unset; if both are provided they must match, ensuring the traced launch occupies the benchmarked position in the post-install JIT/profile ramp |
