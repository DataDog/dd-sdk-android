...
| `coldstart_bench.sh` | the A/B (or A/A) benchmark. Its liveness probe, warm-ups and measured launches all require a numeric `TotalTime` and apply the foreground gate before they can condition or enter the experiment. Each arm's effective runtime-permission outcome is stamped and must remain stable across fresh installs. The achieved dexopt status is likewise read after every compile, required to stay stable across all cells and stamped separately from the requested filter. A successful run ends with exactly one `# RUN COMPLETE`; an interrupted run is not recoverable as a reportable result and must be repeated from the beginning |
| `ab_stats.py` | statistics: paired per-block delta (primary), Welch and permutation tests (diagnostic), per-block drift, block-paired order effect, MDE / required even block counts. `--metric total_ms\|displayed\|ttfd\|app_trace_ms` selects the measurement window. A reportable input must use the complete current harness schema and contain exactly one `# RUN COMPLETE`; there is no unreleased legacy-format fallback. It refuses duplicate aliases or byte-identical copies, incompatible device/protocol metadata, different APKs or arm mappings, SDK-liveness or permission outcomes, runtime controls, launcher components, and incompatible `APP_TRACE_REGEX` identities. `--allow-mixed` acknowledges recorded disagreements but cannot supply missing evidence. It also refuses a malformed or incomplete block/run matrix, rejected or invalid launches, and missing endpoints. `--allow-aborted` and `--allow-missing-endpoint` expose diagnostics only; neither produces a reportable primary interval. With valid evidence, the primary result is gated on stable complementary position pairs and equal baseline-first/treatment-first block counts |
...
A reportable CSV ends with exactly one `# RUN COMPLETE`. The EXIT handler also stamps
