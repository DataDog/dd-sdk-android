...
> not have one. `ALLOW_UNVERIFIED_RADIOS=1` accepts that, and restores nothing for it. They attempt
> to restore those settings on exit, including on Ctrl-C, and make a best-effort reversal of the
> two controls Android cannot read back. Every failed restore is named for manual recovery; a
> disconnected device is never reported as restored. All three workflows revoke only permissions
> they changed from denied to granted, never permissions already granted at install and never
...
- the `perfetto` package, **only** for the trace scripts:
  ...
  your system Python, which will not see the venv. Pass the same flags the capture passes, or
  you read back a weaker verdict than it already produced:
  ```bash
...
| file | role |
|---|---|
| `verify_sdk_active.sh` | **run this first**: proves the SDK actually initializes on-device. Uninstalls/reinstalls, so it deletes app data. Threads are read from **every** process of the package, `<pkg>` and `<pkg>:<private>` alike, so an app that initializes Datadog in a private process is not reported as SDK-less. Exit `0` = live, `1` = not live, `2` = the check could not run. Invalid `SETTLE`, failed installed-APK attestation, failed force-stop, a nonzero launcher command, or launch output without `Status: ok`, `LaunchState: COLD` and a numeric `TotalTime` all use `2`, as do unavailable full process listings and unreadable thread lists: an unverifiable liveness check never borrows the `1` that means the SDK is absent. Its EXIT cleanup preserves that verdict and names any temporary permission grant it could not revoke |
| `coldstart_bench.sh` | the A/B (or A/A) benchmark. Its liveness probe, warm-ups and measured launches all require a successful package-wide `am force-stop`, a numeric `TotalTime`, and the foreground gate before they can condition or enter the experiment. If the stop fails, no launch is attempted because the process-cold precondition is unknown. Each arm's effective runtime-permission outcome is stamped and must remain stable across fresh installs. The achieved dexopt status is likewise read after every compile, required to stay stable across all cells and stamped separately from the requested filter. A successful run ends with exactly one `# RUN COMPLETE`; an interrupted one does not, and any whole counterbalanced prefix it contains is diagnostic only (see "If a run is interrupted") |
| `ab_stats.py` | statistics: paired per-block delta (primary), Welch and permutation tests (diagnostic), per-block drift, block-paired order effect, MDE / required even block counts. `--metric total_ms\|displayed\|ttfd\|app_trace_ms` selects the measurement window. A reportable input must use the complete current harness schema and carry exactly one `# RUN COMPLETE`; there is no unreleased legacy-format fallback. An interrupted run holding at least four whole counterbalanced leading blocks may be inspected over that prefix, but optional or condition-correlated stopping makes it diagnostic only: no primary CI, MDE or significance verdict. It refuses duplicate aliases, byte-identical copies, or inputs carrying the same stable `run_id`; incompatible device/protocol metadata; different APKs; SDK-liveness or permission outcomes; runtime controls; launcher components; and incompatible `APP_TRACE_REGEX` identities. The requested `--baseline` and `--treatment` must match the run's recorded `label_a` and `label_b`, so CLI arguments cannot reverse the sign of the effect. `--allow-mixed` acknowledges supported comparability disagreements but cannot redefine arm roles or supply missing evidence. It also refuses a header the collector cannot have written, including an odd `blocks` (ABBA cannot counterbalance one) or a non-positive `blocks`/`runs`, for every input file and regardless of `--allow-aborted`. It refuses malformed or contradictory evidence by default. `--allow-aborted` and `--allow-missing-endpoint` expose diagnostics only; neither produces a reportable primary interval. With complete valid evidence, the primary result is gated on stable complementary position pairs and equal baseline-first/treatment-first block counts |
...
# capture_trace.sh already verified the trace with these flags. Re-running it by hand is
# for the timestamped detail lines; keep the flags, or you read back a weaker verdict.
./.venv/bin/python verify_trace.py treatment.pftrace --package "$PKG" \
  --require-foreground
```
...
stamp, including the run identifier, must be present. It is read before every gate and changes only
where an expected value comes from, never what is compared.
...
device. `blocks` must be even, for ABBA counterbalancing. Those positional roles are part of
the experiment: `ab_stats.py --baseline` must name `LABEL_A` and `--treatment` must name
`LABEL_B`; swapping them would reverse the reported effect and is refused.
...
| `EXPECTED_COMPILE_STATUS` | *required for trace* | the structured `compile_status` value from the benchmark CSV header, preserving each APK path and ABI assignment. `capture_trace.sh` aborts unless its freshly installed APK reaches the same achieved dexopt state, so a path or ABI changing between `verify` and `speed-profile` cannot be attributed to the SDK |
...
| `APP_TRACE_REGEX` | unset | an ERE matching a log line where your app reports its **own** startup duration; the last number in the match is recorded per launch as `app_trace_ms` and is analyzable with `--metric app_trace_ms`. Use this to A/B the metric your team already quotes, e.g. `APP_TRACE_REGEX='cold_launch total duration: [0-9]+'`. Both the benchmark scrape and `TRACE_ENDPOINT=app_trace_ms` watcher are restricted to the installed package's unique UID, so an unrelated process cannot provide the value; a shared-UID install is refused because it cannot be attributed to one package. The pattern is compile-tested at preflight, treated literally even when it begins with `-`, and passed unchanged to every stale check, scrape and watcher. A malformed ERE aborts immediately instead of yielding `app_trace_ms=NA` on every row of an hour-long run. Check first that the trace actually emits, and that it ends where you think: one app's "first frame" trace ran 140 ms past `am start -W TotalTime` |
...
| `ALLOW_VERSION_MISMATCH` | `0` | `1` acknowledges that the two APKs declare different `versionCode`/`versionName`, or that equality cannot be read because `aapt2` is unavailable or the manifests declare no `versionCode` (two empty version fields would otherwise compare equal and pass a gate that read nothing). Only use it after checking why they differ or confirming their versions by hand; otherwise the SDK is not the only known variable between the arms |
| `ALLOW_UNVERIFIED_PKG` | `0` | `1` bypasses only the APK↔`PKG` check in all three device-touching scripts. Only for the case where `aapt2` is genuinely unavailable or the mismatch is intentional **and** you have confirmed the package by hand. It does not bypass the benchmark's independent APK-version check. Every block runs `adb uninstall $PKG` |
...
**An interrupted run that had already collected whole blocks can be inspected, over exactly those
blocks, but is diagnostic only.** A block counts only as a complete counterbalanced pair of cells:
both arms, all `runs` launches each, every launch passing the gates a measured launch always
passes. The block collection stopped inside is excluded entirely, and so is every launch in it,
which is why the rejected launch that ended the run cannot refuse the analysis that excludes it.
The surviving count is floored to an even number, because an odd one cannot be counterbalanced,
and the block dropped for parity is the last one rather than a chosen one. Four whole blocks is
the floor. It no longer buys an interval, since none is emitted here; it is what keeps the prefix
worth printing at all -- even, so it is counterbalanced, and wide enough that one cell cannot
decide the sign of every delta shown.

The tool prints the shortfall and abort trailer but suppresses the primary CI, MDE and significance
verdict. Rows are visible during collection, so a prefix can be chosen after a favorable result;
automatic failures can also correlate with thermal, storage or other measured conditions. The CSV
does not prove an outcome-independent stopping rule, even when the surviving prefix is balanced.
Re-run the complete registered design for primary inference.

Four cases are still refused outright. Below four whole blocks, because too little survives to
diagnose anything. A run whose declared matrix is whole but which aborted anyway, because nothing
is missing, so the abort happened after collection and this file does not explain it. A run whose
whole blocks are not a consecutive prefix from block 1: the collector runs blocks in order and
stops on an invalid launch, so it cannot leave a whole block after an incomplete one, and a file
that holds one was edited or truncated. And an interrupted run passed alongside other CSVs,
because combining a prefix with a later top-up run creates a different experiment.
`--allow-aborted` still inspects any of them diagnostically, and never emits a primary interval.
...
The checks are independent: `ALLOW_UNVERIFIED_PKG=1` bypasses only the first, and
`ALLOW_VERSION_MISMATCH=1` acknowledges the second or that version equality could not be read.
A missing `aapt2` is a **hard failure** unless both facts were checked and acknowledged, not a
warning. `capture_trace.sh` applies the same package check before its own uninstall.
...
`results_<timestamp>.csv`, one row per launch, warm-ups marked rather than deleted. The header is
whitespace-tokenized when read back, so the values that decide comparability must survive that:
`device` is stamped as an md5 of the model because model names commonly contain spaces, and `fp`,
`abi`, `compile_filter`, `launcher` and `compile_status` are refused outright if they are empty or
hold whitespace rather than being silently truncated to their first word.

```
# device=<full-model-md5> run_id=<collector-run-md5> sdk=... abi=... emulator=0 android_user=0 compile_filter=... compile_status=... perf_mode=... blocks=... runs=... warmup=... animations=0 fp=... launcher=... airplane=0 baseline_md5=... treatment_md5=... label_a=... label_b=... expect_a=0 expect_b=1 app_trace_id=...
...
`device` is the md5 identity of the complete model string (the readable model remains in the
log), so models such as `Pixel 6` and `Pixel 7` cannot both be parsed as `Pixel`. `run_id` is an
opaque identity for one collector invocation. It survives copying or annotating the CSV, which
lets the analyzer reject accidental double-counting even after the bytes differ; it is bookkeeping,
not a tamper-resistance boundary.
...
- Several controls (discarded warm-ups, `am force-stop` rather than reboot, TTID-only,
  pre-granted permissions) bias the measured SDK cost *downward*. The default
  `COMPILE_FILTER=speed-profile` does not: on a fresh install it lands at `verify`, which is
  pessimistic against a steady-state Play install. `COMPILE_FILTER=speed` is the one that biases
  downward. The direction of each is tabulated in the
