...
Four cases are still refused outright. Below four whole blocks, because no honest interval exists.
A run whose declared matrix is whole but which aborted anyway, because nothing is missing, so the
abort happened after collection and this file does not explain it. A run whose whole blocks are
not a consecutive prefix from block 1: the collector runs blocks in order and stops on an invalid
launch, so it cannot leave a whole block after an incomplete one, and a file that holds one was
edited or truncated. And an interrupted run passed alongside other CSVs, because combining a
prefix with a later top-up run lets the operator choose which prefix to report, which is the one
thing this design exists to prevent. `--allow-aborted` still inspects any of them
diagnostically, and never emits a primary interval.
