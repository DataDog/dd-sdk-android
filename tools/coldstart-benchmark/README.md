...
| `ALLOW_VERSION_MISMATCH` | `0` | `1` acknowledges that the two APKs declare different `versionCode`/`versionName`, or that equality cannot be read because `aapt2` is unavailable. Only use it after checking why they differ or confirming their versions by hand; otherwise the SDK is not the only known variable between the arms |
