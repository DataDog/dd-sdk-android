...
| `lib.sh` | shared helpers, so the three device-touching workflows cannot drift into different definitions of the same thing: `adb`/`aapt2` resolution that does not depend on `PATH` and does not trust an `ANDROID_HOME` with no usable build-tools, the Android-user snapshot, the runtime-permission grant, the animation-scale and radio controls, the verified uninstall, the cold-launch validation and the `datadog-*` liveness probe. The latter reports zero only after every discovered process returned a readable thread list; partial enumeration is unknown and aborts |
