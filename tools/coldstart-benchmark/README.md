...
| `EXPECTED_SDK_LIVENESS` | *required for trace* | the selected arm's `expect_a` or `expect_b` value from the benchmark CSV header. It must equal the positional SDK expectation (`0` or `1`), preventing a trace from being labeled as the wrong arm while still passing its own liveness check |
