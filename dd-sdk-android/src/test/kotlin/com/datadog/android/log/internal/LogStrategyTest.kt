...
//        val log = batch.logs
//
//        val jsonObjects = JsonParser.parseString(log).asJsonArray
//        assertLogMatches(jsonObjects[0].asJsonObject, fakeLog)
//    }
...
//    }
    ...
    fun `writes batch of logs`(@Forgery fakeLogs: List<Log>) {
        ...
        val logCount = min(2, fakeLogs.size)
        ...
        val logs = JsonParser.parseString(batch.logs).asJsonArray
        val batchLogCount = min(2, logs.size())
        for (i in 0 until batchLogCount) {
            val jsonObject = logs[i].asJsonObject
