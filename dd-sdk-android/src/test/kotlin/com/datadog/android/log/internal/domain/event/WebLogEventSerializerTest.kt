...
internal class WebLogEventSerializerTest {

    lateinit var testedSerializer: WebLogEventSerializer

    @BeforeEach
    fun `set up`() {
        testedSerializer = WebLogEventSerializer()
    }

    @Test
    fun `serializes full log as json`(@Forgery fakeLog: WebLogEvent) {
        val serialized = testedSerializer.serialize(fakeLog)
        assertSerializedLogMatchesInputLog(serialized, fakeLog)
    }

    @Test
    fun `ignores reserved attributes`(@Forgery fakeLog: WebLogEvent, forge: Forge) {
        // Given
    ...
    @Test
    fun `ignores reserved tags keys`(@Forgery fakeLog: WebLogEvent, forge: Forge) {
        // Given
