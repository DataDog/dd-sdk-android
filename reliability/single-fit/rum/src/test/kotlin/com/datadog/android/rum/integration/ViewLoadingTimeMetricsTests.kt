...
class ViewLoadingTimeMetricsTests {
    ...
    @RepeatedTest(10)
    fun `M provide the TTNS metric for each view event W resource stopped with error, error dropped by mapper`(
        @StringForgery errorMessage: String,
        @Forgery errorSource: RumErrorSource,
        @Forgery throwable: Throwable
    ) {
    ...
    @RepeatedTest(10)
    fun `M not TNS metric W custom identifier, not valid resource`() {
        // Given
        val fakeRumConfiguration = configurationBuilder()
