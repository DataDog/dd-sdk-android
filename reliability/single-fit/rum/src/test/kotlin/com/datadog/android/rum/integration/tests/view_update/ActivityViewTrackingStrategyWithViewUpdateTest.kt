...
import com.datadog.android.rum.integration.ActivityViewTrackingStrategyTest
...
class ActivityViewTrackingStrategyWithViewUpdateTest {
    ...
    @Mock
    lateinit var stubActivity: ActivityViewTrackingStrategyTest.StubActivity
    ...
    fun `M send RUM View W onActivityResumed()`() {
        ...
        // Then
        val eventsWritten = stubSdkCore.eventsWritten(Feature.Companion.RUM_FEATURE_NAME)
        StubEventsAssert.Companion.assertThat(eventsWritten)
            .hasSize(1)
            .hasRumEvent(index = 0) {
    ...
    fun `M not send RUM View update W onActivityStopped() {start not tracked}`() {
        ...
        // Then
        val eventsWritten = stubSdkCore.eventsWritten(Feature.Companion.RUM_FEATURE_NAME)
        StubEventsAssert.Companion.assertThat(eventsWritten).hasSize(0)
    }
