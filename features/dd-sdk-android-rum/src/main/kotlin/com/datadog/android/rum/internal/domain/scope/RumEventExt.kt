...
// region RumViewState conversions

internal fun NetworkInfo.toViewStateConnectivity(): RumViewState.Connectivity {
    val status = if (isConnected()) {
        RumViewState.ConnectivityStatus.CONNECTED
    } else {
        RumViewState.ConnectivityStatus.NOT_CONNECTED
    }
    val interfaces = when (connectivity) {
        NetworkInfo.Connectivity.NETWORK_ETHERNET -> listOf(RumViewState.Interface.ETHERNET)
        NetworkInfo.Connectivity.NETWORK_WIFI -> listOf(RumViewState.Interface.WIFI)
        NetworkInfo.Connectivity.NETWORK_WIMAX -> listOf(RumViewState.Interface.WIMAX)
        NetworkInfo.Connectivity.NETWORK_BLUETOOTH -> listOf(RumViewState.Interface.BLUETOOTH)
        NetworkInfo.Connectivity.NETWORK_2G,
        NetworkInfo.Connectivity.NETWORK_3G,
        NetworkInfo.Connectivity.NETWORK_4G,
        NetworkInfo.Connectivity.NETWORK_5G,
        NetworkInfo.Connectivity.NETWORK_MOBILE_OTHER,
        NetworkInfo.Connectivity.NETWORK_CELLULAR -> listOf(RumViewState.Interface.CELLULAR)

        NetworkInfo.Connectivity.NETWORK_OTHER -> listOf(RumViewState.Interface.OTHER)
        NetworkInfo.Connectivity.NETWORK_NOT_CONNECTED -> emptyList()
    }

    val cellular = if (cellularTechnology != null || carrierName != null) {
        RumViewState.Cellular(
            technology = cellularTechnology,
            carrierName = carrierName
        )
    } else {
    ...
    return RumViewState.Connectivity(
        status,
        interfaces,
        cellular = cellular
    )
...
internal fun DeviceType.toViewStateDeviceType(): RumViewState.DeviceType {
    return when (this) {
        DeviceType.MOBILE -> RumViewState.DeviceType.MOBILE
        DeviceType.TABLET -> RumViewState.DeviceType.TABLET
        DeviceType.TV -> RumViewState.DeviceType.TV
        DeviceType.DESKTOP -> RumViewState.DeviceType.DESKTOP
        DeviceType.GAMING_CONSOLE -> RumViewState.DeviceType.GAMING_CONSOLE
        DeviceType.BOT -> RumViewState.DeviceType.BOT
        DeviceType.OTHER -> RumViewState.DeviceType.OTHER
    }
}

internal fun RumSessionScope.StartReason.toViewStateSessionPrecondition(): RumViewState.SessionPrecondition {
    return when (this) {
        RumSessionScope.StartReason.USER_APP_LAUNCH -> RumViewState.SessionPrecondition.USER_APP_LAUNCH
        RumSessionScope.StartReason.INACTIVITY_TIMEOUT -> RumViewState.SessionPrecondition.INACTIVITY_TIMEOUT
        RumSessionScope.StartReason.MAX_DURATION -> RumViewState.SessionPrecondition.MAX_DURATION
        RumSessionScope.StartReason.EXPLICIT_STOP -> RumViewState.SessionPrecondition.EXPLICIT_STOP
        RumSessionScope.StartReason.BACKGROUND_LAUNCH -> RumViewState.SessionPrecondition.BACKGROUND_LAUNCH
        RumSessionScope.StartReason.PREWARM -> RumViewState.SessionPrecondition.PREWARM
        RumSessionScope.StartReason.FROM_NON_INTERACTIVE_SESSION ->
            RumViewState.SessionPrecondition.FROM_NON_INTERACTIVE_SESSION
    }
}

internal fun String.toViewStateSource(): RumViewState.ViewEventSource? {
    return try {
        when (ViewEvent.ViewEventSource.fromJson(this)) {
            ...
            ViewEvent.ViewEventSource.KOTLIN_MULTIPLATFORM ->
                RumViewState.ViewEventSource.KOTLIN_MULTIPLATFORM
