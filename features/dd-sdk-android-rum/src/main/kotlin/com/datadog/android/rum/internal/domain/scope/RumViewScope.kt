...
internal open class RumViewScope(
    ...
    override fun handleEvent(
        ...
        when (event) {
            ...
            is RumRawEvent.RetryFeatureOperation -> onRetryFeatureOperation(event, writer)
            is RumRawEvent.UpdateFeatureOperation -> onUpdateFeatureOperation(event, writer)
    ...
    private fun onRetryFeatureOperation(event: RumRawEvent.RetryFeatureOperation, writer: DataWriter<Any>) {
        sdkCore.newRumEventWriteOperation(writer) { datadogContext ->
            newVitalStepEvent(
                datadogContext,
                name = event.name,
                operationKey = event.operationKey,
                stepType = RumVitalEvent.StepType.RETRY,
                failureReason = null
            )
        }
        sendViewUpdate(event, writer)
    ...
    private fun onUpdateFeatureOperation(event: RumRawEvent.UpdateFeatureOperation, writer: DataWriter<Any>) {
        sdkCore.newRumEventWriteOperation(writer) { datadogContext ->
            newVitalStepEvent(
                datadogContext,
                name = event.name,
                operationKey = event.operationKey,
                stepType = RumVitalEvent.StepType.UPDATE,
                failureReason = null
            )
        }
        sendViewUpdate(event, writer)
    ...
    private fun newVitalStepEvent(
        ...
        val vitalFailureReason = failureReason?.let {
            when (it) {
        ...
        return RumVitalEvent(
            ...
            vital = RumVitalEvent.RumVitalEventVital(
                ...
                stepType = stepType,
                failureReason = vitalFailureReason,
