...
internal class RumActionScopeTest {
    ...
    @Test
    fun `M populate heatmap fields W sendAction() {componentId present}`(
        @StringForgery fakeComponentId: String,
        ...
        val attributesWithDimensions = fakeAttributes +
            (RumAttributes.INTERNAL_ACTION_COMPONENT_ID to fakeComponentId) +
        ...
        argumentCaptor<ActionEvent> {
            verify(mockWriter).write(eq(mockEventBatchWriter), capture(), eq(EventType.DEFAULT))
            assertThat(lastValue)
                .hasComponentId(fakeComponentId)
    ...
    @Test
    fun `M have null componentId W sendAction() {targetSelector is null}`(
        @LongForgery fakeWidth: Long,
        @LongForgery fakeHeight: Long,
        @LongForgery fakePosX: Long,
        @LongForgery fakePosY: Long
