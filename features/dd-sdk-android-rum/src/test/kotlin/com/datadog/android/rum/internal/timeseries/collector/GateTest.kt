...
internal class GateTest {
    ...
    fun `M declare foreground volatile W inspect field modifiers`() {
        // When
        val modifiers = Gate::class.java.getDeclaredField("isForeground").modifiers

        // Then
        assertThat(Modifier.isVolatile(modifiers)).isTrue()
    }

    // region getRumContext() / runIfGateOpen()
    ...
    @Test
    fun `M invoke runnable W runIfGateOpen() { generation matches, allowed }`() {
        // Given
        testedGate.setForeground(true)
        testedGate.setRumContext(fakeRumContextOf(RumViewType.FOREGROUND)) { _, _ -> }
        var generation = -1
        ...
        val executed = testedGate.runIfGateOpen(generation) { received = it }
    ...
    @Test
    fun `M not invoke runnable W runIfGateOpen() { stale generation }`() {
        // Given
        testedGate.setForeground(true)
        var generation = 0
        testedGate.setSessionActive(true) { newGeneration, _ -> generation = newGeneration }
        ...
        // When
        var invoked = false
        val executed = testedGate.runIfGateOpen(staleGeneration) { invoked = true }
    ...
    @Test
    fun `M not invoke runnable W runIfGateOpen() { not allowed }`() {
        // When
        var invoked = false
        val executed = testedGate.runIfGateOpen(0) { invoked = true }
