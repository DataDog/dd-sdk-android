...
internal class RumViewScopeTest {
    ...
    var fakeVitalSourceEvent: VitalOperationStepEvent.VitalOperationStepEventSource? = null
    ...
    fun `set up`(forge: Forge) {
        ...
        fakeVitalSourceEvent = if (isValidSource) {
            VitalOperationStepEvent.VitalOperationStepEventSource.fromJson(fakeSource)
        } else {
            null
        }
    ...
    fun `M force profiling status RUNNING in ErrorEvent W handleEvent(AddError) {triggered by profiling}`(
        ...
        // was necessarily running, even though the generic profiling feature context says otherwise.
        val throwable = ANRException(Thread.currentThread())
        testedScope.activeActionScope = mockActionScope
        val attributes = forge.exhaustiveAttributes(excludedKeys = fakeAttributes.keys) +
