...
import com.datadog.android.rum.internal.domain.scope.toTimeseriesCpuSchemaType
...
internal class CpuEventFactoryTest {
    ...
    private var fakeSource: TimeseriesCpuEvent.Source? = null
    ...
    fun `set up`(forge: Forge) {
        fakeSource = forge.aNullable { aValueFrom(TimeseriesCpuEvent.Source::class.java) }
    ...
    fun `M return null W create() { empty input }`() {
        // When
        val result = testedFactory().create(fakeDatadogContext, fakeSerializedRumContext, emptyList())
    ...
    fun `M fill timeseries W create() { multiple data points }`(
        ...
        // Then
        TimeseriesCpuEventAssert.assertThat(event)
            .hasTimeseriesName(VALUE_NAME_CPU)
    ...
    fun `M map session type W create() { synthetics }`() {
        ...
        // Then
        TimeseriesCpuEventAssert.assertThat(event).hasSessionType(TimeseriesCpuEventSessionType.SYNTHETICS)
    }
    ...
    @Test
    fun `M fill cpu_usage W create() { one value per data point }`(
        @DoubleForgery(min = 1.0, max = 100.0) fakeFirstCpu: Double,
        @DoubleForgery(min = 1.0, max = 100.0) fakeSecondCpu: Double,
        @LongForgery(min = 1L) fakeTs: Long
    ) {
        // Given
        val samples = listOf(
            DataPoint(fakeTs, fakeFirstCpu),
            DataPoint(fakeTs + 1L, fakeSecondCpu)
        )
        ...
        TimeseriesCpuEventAssert.assertThat(event)
            .hasDataPointsCount(2)
            .hasCpuUsage(fakeFirstCpu, position = 0)
            .hasCpuUsage(fakeSecondCpu, position = 1)
    ...
    fun `M fill os and device W create()`() {
        ...
        assertThat(device.type).isEqualTo(deviceInfo.deviceType.toTimeseriesCpuSchemaType())
    ...
    fun `M fill dd W create()`(
        ...
        assertThat(event.dd.session?.sessionPrecondition)
            .isEqualTo(TimeseriesCpuEvent.SessionPrecondition.INACTIVITY_TIMEOUT)
    ...
    ): TimeseriesCpuEvent = TimeseriesCpuEvent.fromJson(
        checkNotNull(
            testedFactory(sessionType = sessionType)
                .create(datadogContext, rumContext, dataPoints)
        ).toJson().toString()
    )
    ...
    private fun createEvent(
        ...
        rumContext: RumContext = fakeSerializedRumContext,
        sessionType: RumSessionType = RumSessionType.USER
    ): TimeseriesCpuEvent = checkNotNull(
        testedFactory(sessionType = sessionType)
            .create(datadogContext, rumContext, oneSample)
    )

    companion object {
        private const val VALUE_NAME_CPU: String = "cpu"
