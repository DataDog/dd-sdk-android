...
internal class DatadogRumMonitorTest {
    ...
    @OptIn(ExperimentalRumApi::class)
    @Test
    fun `M produce UpdateFeatureOperation event W updateFeatureOperation`(
        @StringForgery key: String,
        @StringForgery name: String,
        forge: Forge
    ) {
        ...
        assertMethodCallProducesValidEvent<RumRawEvent.UpdateFeatureOperation>(
            whenCalled = {
                testedMonitor.updateFeatureOperation(name, operationKey, attributes)
            },
            then = { event ->
                assertThat(event.name).isEqualTo(name)
                assertThat(event.operationKey).isEqualTo(operationKey)
    ...
    @OptIn(ExperimentalRumApi::class)
    @Test
    fun `M produce RetryFeatureOperation event W retryFeatureOperation`(
        @StringForgery key: String,
        @StringForgery name: String,
        forge: Forge
    ) {
        ...
        assertMethodCallProducesValidEvent<RumRawEvent.RetryFeatureOperation>(
            whenCalled = {
                testedMonitor.retryFeatureOperation(name, operationKey, attributes)
            },
            then = { event ->
                assertThat(event.name).isEqualTo(name)
                assertThat(event.operationKey).isEqualTo(operationKey)
