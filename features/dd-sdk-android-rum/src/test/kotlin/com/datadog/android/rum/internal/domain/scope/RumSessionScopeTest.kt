...
internal class RumSessionScopeTest {
    ...
    fun `M call reportTTID and create new session W handleEvent { AppLaunchTTIDEvent }`(
        ...
        val event = RumRawEvent.AppLaunchTTIDEvent(
            eventTime = Time(),
            info = info
        )
        ...
        // Then
        val context1 = result!!.getRumContext()

        assertThat(result).isSameAs(testedScope)
        assertThat(context1.sessionId).isNotEqualTo(RumContext.NULL_UUID)
        assertThat(context1.sessionState).isEqualTo(RumSessionScope.State.TRACKED)
        assertThat(context1.sessionStartReason).isEqualTo(RumSessionScope.StartReason.USER_APP_LAUNCH)
    ...
    fun `M call reportTTID twice { AppLaunchTTIDEvent }`(
        ...
        val event1 = RumRawEvent.AppLaunchTTIDEvent(
            eventTime = Time(),
            info = info1
        )

        val event2 = RumRawEvent.AppLaunchTTIDEvent(
            eventTime = Time(),
            info = info2
        )
