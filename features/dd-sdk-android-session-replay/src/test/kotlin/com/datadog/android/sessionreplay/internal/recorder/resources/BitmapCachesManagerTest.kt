...
internal class BitmapCachesManagerTest {
    ...
    fun `M put in resource cache W putInResourceCache`() {
        ...
        // Then
        verify(mockResourcesCache).put(mockDrawable, CacheData(fakeResourceId.toByteArray(Charsets.UTF_8)))
    }

    @Test
    fun `M get resource from resource cache W getFromResourceCache { resource exists in cache }`() {
        // Given
        val fakeCacheData = CacheData(fakeResourceId.toByteArray(Charsets.UTF_8))
    ...
    private fun createBitmapCachesManager(
        ...
        resourcesLRUCache: Cache<Drawable, CacheData>,
