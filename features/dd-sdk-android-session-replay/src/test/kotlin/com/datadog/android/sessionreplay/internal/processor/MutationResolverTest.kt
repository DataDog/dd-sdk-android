...
internal class MutationResolverTest {
    ...
    fun `M identify the updated wireframes W resolveMutations {EmbeddedContent removed at beginning}`(forge: Forge) {
        ...
        val fakeHiddenEmbeddedContents = forge.anInt(min = 1, max = fakePrevSnapshot.size - 1)
        val fakeCurrentSnapshot = fakePrevSnapshot.drop(fakeHiddenEmbeddedContents)
        val expectedUpdates = fakePrevSnapshot.take(fakeHiddenEmbeddedContents).map {
    ...
    fun `M identify the updated wireframes W resolveMutations {EmbeddedContent removed at end}`(forge: Forge) {
        ...
        val fakeHiddenEmbeddedContents = forge.anInt(min = 1, max = fakePrevSnapshot.size - 1)
        val fakeCurrentSnapshot = fakePrevSnapshot.dropLast(fakeHiddenEmbeddedContents)
        val expectedUpdates = fakePrevSnapshot.takeLast(fakeHiddenEmbeddedContents).map {
            MobileSegment.WireframeUpdateMutation.EmbeddedContentWireframeUpdate(
                id = it.id(),
                slotId = it.slotId,
                isVisible = false
