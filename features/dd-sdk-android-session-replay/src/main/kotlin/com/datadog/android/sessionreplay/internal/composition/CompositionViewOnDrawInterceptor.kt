...
internal class CompositionViewOnDrawInterceptor(
    ...
    /**
     * [intercept] only decides which views are new under [lock]; the actual registration below
     * must reserve the view under the same lock before touching the observer; otherwise two
     * concurrent [intercept] calls that both see the same view as new would both register a
     * listener for it, and only one would ever be found again to remove.
     */
    private fun addListener(view: View) {
        ...
        try {
            ...
            synchronized(lock) { interceptedViews.remove(view) }
