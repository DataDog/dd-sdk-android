...
internal object RumContextTestsUtils {
    ...
    fun Forge.aDatadogContextWithRumContext(rumContext: Map<String, Any?>): DatadogContext =
        getForgery<DatadogContext>().let {
            it.copy(featuresContext = it.featuresContext + mapOf(Feature.RUM_FEATURE_NAME to rumContext))
        }

    fun OngoingStubbing<Unit>.thenReturnContext(datadogContext: DatadogContext) {
        doAnswer {
            it.getArgument<(DatadogContext) -> Unit>(it.arguments.lastIndex).invoke(datadogContext)
        }
    }
}
