...
internal class DatadogPropagationHelperTest {
    ...
    @Mock
    lateinit var mockRequestModifier: HttpRequestInfoBuilder
    ...
    fun `set up`() {
        ...
        whenever(mockRequestModifier.build()) doReturn mock()
    }
    ...
    fun `M return span context from tag W extractParentContext() {DatadogSpan tag}`() {
        ...
        whenever(
            mockPropagation.extract(
                eq(mockRequest),
                any<(HttpRequestInfo, (String, String) -> Boolean) -> Unit>()
            )
        ).doReturn(null)
    ...
    private fun setupInjectorCallback(expectedKey: String, expectedValue: String) {
        whenever(
            mockPropagation.inject(
                ...
                any<(HttpRequestInfoBuilder, String, String) -> Unit>()
            )
        ).doAnswer { invocation ->
            @Suppress("UNCHECKED_CAST")
            val injector = invocation.arguments[2] as (HttpRequestInfoBuilder, String, String) -> Unit
