...
def copy_generated_wiki(root, module_name: str, repoPath: str, sidebar_file: TextIO):
    wiki_path = os.path.join(root, module_name, "build", "wiki")
    index_path = os.path.join(root, module_name, "build", "wiki", module_name + ".md")
...
def update_wiki(gh_token: str, dry_run: bool) -> int:
    ...
    roots = [".", "features", "integrations"]
    for root in roots:

        print("Checking root : " + root)
        modules = sorted([f.name for f in os.scandir(root) if f.is_dir()])
        print(str(modules))
        for module in modules:
            copy_generated_wiki(root, module, temp_dir_path, sidebar_file)
