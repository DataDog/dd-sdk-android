# FrameStatesAggregator onFrame Microbenchmark (Local Emulator)
...
- The `timeNs` regression (+121%) has very narrow CIs (~3% band), indicating low noise for this benchmark on the local emulator.
...
The local emulator **reliably detects both regressions** in this benchmark. The ~121% timing regression and the +1 allocation regression are both detected with high confidence. The `onFrame` microbenchmark has excellent noise characteristics on emulators, making it suitable for automated regression detection even for moderate (~2x) performance changes.
