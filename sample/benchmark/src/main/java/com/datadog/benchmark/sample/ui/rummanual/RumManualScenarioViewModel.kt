...
internal sealed interface RumManualScenarioScreenAction {
    ...
    data class OneEventBatchSent(val task: RumManualScenarioState.EventSendingTask) : RumManualScenarioScreenAction
    data class EventSendingTaskFinished(val task: RumManualScenarioState.EventSendingTask) : RumManualScenarioScreenAction
}
...
internal class RumManualScenarioViewModel(
    ...
    private suspend fun sendBatch(task: RumManualScenarioState.EventSendingTask) {
        repeat(task.config.eventsPerBatch) {
            ...
            dispatch(RumManualScenarioScreenAction.OneEventBatchSent(task))
        }
    }
    ...
    private suspend fun withRumView(
        config: RumManualScenarioState.Config,
        block: suspend () -> Unit
    ) {
        rumMonitor.startView(key = this@RumManualScenarioViewModel, name = config.viewName)
        block()
        rumMonitor.stopView(key = this@RumManualScenarioViewModel)
    }
    ...
    private fun processEventsSent(
        ...
        return when (action) {
            is RumManualScenarioScreenAction.OneEventBatchSent -> {
                prev.eventsSent + 1
            }
