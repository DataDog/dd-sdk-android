...
sealed interface RumAutoCharactersScreenAction {
...
data class RumAutoCharactersScreenState(
    val pages: List<Page>,
    ...
    val allCharacters by lazy {
        pages.flatMap { it.response.results }
    }
}

class RumAutoCharactersViewModel(
    ...
    private fun processAction(
        prev: RumAutoCharactersScreenState,
        action: RumAutoCharactersScreenAction
    ): RumAutoCharactersScreenState {
    ...
    private fun processTask(
        prev: RumAutoCharactersScreenState,
        action: RumAutoCharactersScreenAction
    ): Pair<PageLoadingTask, PageLoadingTaskResult>? {
        return when (action) {
    ...
    private fun processPages(
        prev: RumAutoCharactersScreenState,
        action: RumAutoCharactersScreenAction
    ): List<RumAutoCharactersScreenState.Page> {
        return when (action) {
            is RumAutoCharactersScreenAction.PageLoadingFinished -> {
                val newPage = action.response.optionalResult?.let { characters ->
                    RumAutoCharactersScreenState.Page(
                        characters
                    )
                }
                if (newPage != null) {
