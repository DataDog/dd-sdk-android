...
fun InternalLogger.verifyApiUsage(
    expectedApiUsageEvent: InternalTelemetryEvent.ApiUsage,
    expectedSamplingRate: Float = 15f,
    mode: VerificationMode = times(1)
) {
    argumentCaptor<() -> InternalTelemetryEvent.ApiUsage> {
        verify(this@verifyApiUsage, mode).logApiUsage(
            eq(expectedSamplingRate),
            capture()
        )
        allValues.forEach {
            assertThat(it()).usingRecursiveComparison().isEqualTo(expectedApiUsageEvent)
        }
    }
}
