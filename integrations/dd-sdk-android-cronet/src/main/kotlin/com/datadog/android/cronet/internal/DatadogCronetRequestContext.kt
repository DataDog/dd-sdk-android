...
internal class DatadogCronetRequestContext internal constructor(
    ...
    internal fun <T> setTag(type: Class<in T>, tag: T?) {
        if (tag == null) {
            additionalAnnotations.removeAll { it.javaClass == type }
        } else {
    ...
    internal fun addRequestAnnotation(annotation: Any) {
        val existingIndex = additionalAnnotations.indexOf(annotation)
        if (existingIndex >= 0) {
            additionalAnnotations[existingIndex] = annotation
        } else {
...
private fun UrlRequest.Builder.applyRequestParams(params: CronetRequestParams) = apply {
    ...
    params.rawCompressionDictionary?.let {
        setRawCompressionDictionary(
            it.dictionarySha256Hash,
            it.dictionary,
            it.dictionaryId
        )
    }
    params.uploadDataProviderParams?.let { setUploadDataProvider(it.uploadDataProvider, it.executor) }
}

internal data class CronetRequestParams(
    ...
    class RawCompressionDictionary(
