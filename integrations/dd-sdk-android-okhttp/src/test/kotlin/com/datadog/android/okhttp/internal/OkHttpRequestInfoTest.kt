...
internal class OkHttpRequestInfoTest {
    ...
    fun `M delegate W url property`(@StringForgery fakeUrl: String) {
        ...
        // When
        val result = OkHttpRequestInfo(mockRequest, mockInternalLogger).url
    ...
    fun `M delegate W method property`(@StringForgery fakeMethod: String) {
        ...
        // When
        val result = OkHttpRequestInfo(mockRequest, mockInternalLogger).method
    ...
    fun `M delegate W headers property`(forge: Forge) {
        ...
        // When
        val result = OkHttpRequestInfo(mockRequest, mockInternalLogger).headers
    ...
    fun `M delegate W contentType property { content type present }`(
        ...
        // When
        val result = OkHttpRequestInfo(mockRequest, mockInternalLogger).contentType
    ...
    fun `M delegate W tag`(@StringForgery fakeTag: String) {
        ...
        // When
        val tag = OkHttpRequestInfo(mockRequest, mockInternalLogger).tag(String::class.java)
    ...
    fun `M delegate W contentLength() { request body has length }`(@LongForgery fakeContentLength: Long) {
        ...
        // When
        val result = OkHttpRequestInfo(mockRequest, mockInternalLogger).contentLength()
    ...
    fun `M stop the body from producing the discarded part W peekBody() { body over the limit }`() {
        ...
        var writtenChunks = 0
        lateinit var peekSink: BufferedSink
        val fakeBody = object : RequestBody() {
            override fun contentType(): MediaType? = null
            override fun writeTo(sink: BufferedSink) {
                peekSink = sink
        ...
        assertThat(peekSink.buffer.size).isZero()
    }
