...
internal open class TracingInterceptorTest {
    ...
    @Test
    fun `M prevent infinite loop W listener retries same request multiple times`(
        @IntForgery(min = 200, max = 300) statusCode: Int
    ) {
        // Given
        var interceptCallCount = 0
        val maxRetries = TracingInterceptor.MAX_REQUEST_INTERCEPTION_COUNT
        ...
        whenever(
            ...
            interceptCallCount++
            if (interceptCallCount <= maxRetries + 1) {
                testedInterceptor.intercept(mockChain)
            }
            return@doAnswer Unit
        }
        ...
        // Then
        verify(mockRequestListener, times(maxRetries)).onRequestIntercepted(any(), any(), anyOrNull(), anyOrNull())
    ...
    @Test
    fun `M reset counter W listener succeeds without retry`(
        @IntForgery(min = 200, max = 300) statusCode: Int
    ) {
        // Given
        var callCount = 0
        ...
        verify(mockRequestListener, times(3)).onRequestIntercepted(any(), any(), anyOrNull(), anyOrNull())
    ...
    @Test
    fun `M prevent infinite loop W listener does sequential retries with same request`(
        @IntForgery(min = 200, max = 300) statusCode: Int
    ) {
        // Given
        var interceptCallCount = 0
