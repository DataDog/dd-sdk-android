...
internal fun identifyRequest(request: Request): String {
    ...
    return if (body == null) {
        ...
        val contentLength = try {
            body.contentLength()
        } catch (@Suppress("SwallowedException") _: java.io.IOException) {
            0
        }
...
internal class OkHttpRequestInfo(
    ...
    override fun peekBody(maxBytes: Long): HttpBodySnapshot? {
        ...
        return try {
            ...
            try {
                ...
                // Truncation interrupts forwarding and can leave discarded bytes in the wrapper.
                // Do not close it: close() would attempt to forward those bytes again.
                @Suppress("UnsafeThirdPartyFunctionCall") // the buffer is owned by bufferedSink
                bufferedSink.buffer.clear()
            }
    ...
    override fun newBuilder() = OkHttpRequestInfoBuilder(
        originalRequest.newBuilder(),
        internalLogger
    )

    internal companion object {
        const val ERROR_PEEK_REQUEST_BODY = "Unable to peek request body."
    }
}
 ...
 * @param internalLogger the logger used when creating request information.
 */
@InternalApi
