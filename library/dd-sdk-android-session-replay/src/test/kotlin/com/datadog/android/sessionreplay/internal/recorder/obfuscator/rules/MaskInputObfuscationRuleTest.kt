...
internal class MaskInputObfuscationRuleTest : BaseObfuscationRuleTest() {

    lateinit var testedRule: MaskInputObfuscationRule

    @BeforeEach
    fun `set up`() {
        super.setUp()
        testedRule = MaskInputObfuscationRule(
    ...
    @EnumSource(
        TextType::class,
        names = ["INPUT_TEXT", "SENSITIVE_TEXT", "OPTION_TEXT"],
        mode = EnumSource.Mode.EXCLUDE
    )
    fun `M resolve non obfuscated text W resolveObfuscatedValue { non input or sensitive text }`(
        textType: TextType
    ) {
        // Given
        whenever(mockTextTypeResolver.resolveTextType(mockTextView, fakeMappingContext))
