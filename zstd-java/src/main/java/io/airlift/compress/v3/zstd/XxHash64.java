...
import static io.airlift.compress.v3.zstd.UnsafeUtil.ARRAY_BYTE_BASE_OFFSET;
...
final class XxHash64
{
    ...
    private void updateHash(Object base, long address, int length)
    {
        if (bufferSize > 0) {
            ...
            UnsafeUtil.copyMemory(base, address, buffer, BUFFER_ADDRESS + bufferSize, available);
        ...
        if (length > 0) {
            UnsafeUtil.copyMemory(base, address, buffer, BUFFER_ADDRESS, length);
            bufferSize = length;
        }
    }
